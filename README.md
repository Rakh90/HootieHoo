...
- **Groups** — organize members into named groups (e.g. Family, Hiking Buddies) and toggle a whole group on or off at once, either from the Members tab or with one tap on a group's chip right on the Send dashboard. A member can belong to **more than one group** at a time (tap all that apply when adding or editing someone) — a message reaches them as long as *at least one* of their groups is switched on, so someone in both Family and Hiking Buddies still gets a text even if you've toggled Family off for the day. New installs start with one group ("My Group") so there's always somewhere to put people, and every member must belong to at least one. Deleting a group moves its members into another group rather than deleting them (blocked entirely if it's your only group).
- **Members** — add people with a phone number and/or email, tick which group(s) they belong to, and flip any one of them off without deleting them — handy for "not this trip" without losing their info. On browsers that support it (currently Chrome on Android), an **Add from Contacts** button lets you pick people straight from your phone's contact list instead of typing them in — see below for where this does and doesn't work.
- **Settings** — edit both message texts and button labels, and choose whether messages go out as a group text (SMS) or email (bcc). Email is a useful fallback for a contact without a phone number, or when group SMS gets flaky across a mix of iPhone and Android. Also shows the app's own link with **Share link** / **Copy link** buttons, so you can easily send your group the URL — handy for letting them know what a HootieHoo text actually is. "Share link" opens the phone's native share sheet where available (iOS Safari and Android Chrome both support this); where it isn't, it falls back to a blank text message with the link already typed in.

Both the Members and Settings tabs are menu-first: you land on a short list of categories (Groups / each individual group / Add a Member; Messages / Share App / Data) and tap into whichever one you need, with a Back button to return. Nothing dumps every panel onto one long scrolling screen at once — that's deliberate, so a page with a lot of groups or a lot of settings stays easy to scan instead of turning into a wall of cards.
